...
> 28 task JSON files in `tasks/` (1 seed + 27 generated). All pass `validate-tasks.mjs`.
> Fan-out: none (no explicitly enumerated fan-out dimensions in the backlog).

| File | Title | Milestone | Type | Risk | Status |
| --- | --- | --- | --- | --- | --- |
| `tasks/rare-cancer-registry-templates-gov-001.json` | Standards-not-a-data-store + synthetic-data-only constitution | M0 | design-spec | medium | open |
| `tasks/rare-cancer-registry-templates-license-001.json` | Source allow-list schema + licence-gate policy (open-lead, restricted-stub rule) | M0 | design-spec | medium | open |
| `tasks/rare-cancer-registry-templates-license-002.json` | Analyse & record ≥5 candidate standards; approve the open core | M0 | research | medium | open |
| `tasks/rare-cancer-registry-templates-prov-001.json` | Provenance model + countable assertion (administered-item) unit | M0 | design-spec | low | open |
| `tasks/rare-cancer-registry-templates-ci-001.json` | CI scaffold: provenance linter + licence deny-list + 'no real data' scan | M0 | code | low | open |
| `tasks/rare-cancer-registry-templates-partner-001.json` | Reviewer + adopting-community outreach (oncologist, advocate, bioethics/legal, licence) | M0 | research | low | open |
| `tasks/rare-cancer-registry-templates-cde-001.json` | Minimal-core CDE set in LinkML (de-identified-by-design) | M1 | design-spec | medium | open |
| `tasks/rare-cancer-registry-templates-vocab-001.json` | Open-vocabulary bindings + restricted-terminology crosswalk stubs | M1 | data | medium | open |
| `tasks/rare-cancer-registry-templates-crosswalk-001.json` | Crosswalks CDE ⇄ mCODE/FHIR, OMOP CDM, GA4GH Phenopackets | M1 | data | medium | open |
| `tasks/rare-cancer-registry-templates-fixtures-001.json` | Synthetic conformance fixtures + round-trip test | M1 | code | low | open |
| `tasks/rare-cancer-registry-templates-validate-001.json` | Validator CLI (CDE schema + crosswalk round-trip) | M1 | code | low | open |
| `tasks/rare-cancer-registry-templates-consent-001.json` | Consent templates (broad/tiered/dynamic) + FHIR Consent model w/ revocation + DUO | M2 | design-spec | high | open |
| `tasks/rare-cancer-registry-templates-consent-002.json` | Deceased-patient / memorial-registry consent pattern + ethics notes | M2 | writing | high | open |
| `tasks/rare-cancer-registry-templates-governance-001.json` | Governance-charter + data-sharing-agreement + FAIR/CARE checklist templates | M2 | design-spec | medium | open |
| `tasks/rare-cancer-registry-templates-compliance-001.json` | Informational GDPR/HIPAA/Common-Rule mapping (primary-source-cited) | M2 | research | high | open |
| `tasks/rare-cancer-registry-templates-govcheck-001.json` | Governance-conformance checklist in the validator | M2 | code | medium | open |
| `tasks/rare-cancer-registry-templates-ig-001.json` | FHIR ImplementationGuide + LinkML-generated docs starter-kit | M3 | code | medium | open |
| `tasks/rare-cancer-registry-templates-testbank-001.json` | Synthetic conformance test-bank (CDE + crosswalk + governance) | M3 | data | low | open |
| `tasks/rare-cancer-registry-templates-adopt-001.json` | Secure adopting community; stand up/harmonise a consent-first registry against the standard | M3 | research | medium | open |
| `tasks/rare-cancer-registry-templates-sustain-001.json` | Sustainability: version policy, staleness cadence, upstream-contribution + reviewer rotation plan | M3 | writing | low | open |
| `tasks/rare-cancer-registry-templates-extend-001.json` | Per-disease extension pattern, validated on one exemplar subtype | M4 | design-spec | medium | open |
| `tasks/rare-cancer-registry-templates-upstream-001.json` | Upstream-contribution path (OHDSI/GA4GH/ERN) + maintenance rotation | M4 | maintenance | low | open |
| `tasks/rare-cancer-registry-templates-i18n-001.json` | Multilingual consent/explainer templates (plain-language) | Backlog | writing | high | open |
| `tasks/rare-cancer-registry-templates-dynconsent-001.json` | Reference (non-data-holding) dynamic-consent state schema | Backlog | design-spec | medium | open |
| `tasks/rare-cancer-registry-templates-care-001.json` | CARE-principles / Indigenous data-governance template module | Backlog | writing | medium | open |
| `tasks/rare-cancer-registry-templates-deid-001.json` | De-identification design guidance (Safe Harbor / Expert Determination) | Backlog | writing | high | open |
| `tasks/rare-cancer-registry-templates-omopvocab-001.json` | Open-only OMOP vocabulary mapping pack | Backlog | data | medium | open |
| `tasks/rare-cancer-registry-templates-jurmatrix-001.json` | Deceased-data jurisdictional rules matrix | Backlog | research | high | open |
